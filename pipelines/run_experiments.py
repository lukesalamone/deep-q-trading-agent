...
def run_training(model: DQN, index: str, symbol: str, train_dataset: str, valid_dataset: str,
                 strategy: int, path:str, splits, episodes: int=config['EPISODES']):
...
def run_experiment(**experiment):
    ...
    if task == 'transfer_learning':
        ...
        if task == 'evaluate':
            ...
            model = run_training(model=model, index=index, episodes=episodes, symbol=symbol, train_dataset=train_set, valid_dataset=eval_set,
